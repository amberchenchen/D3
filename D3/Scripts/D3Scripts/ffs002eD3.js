 ...
 * Resolve `path`.
 * 
 * Lookup: - PATH/index.js - PATH.js - PATH
 * 
 * @param {String}
 *            path
    ...
    _createElement: function() {
        ...
        if (!found) {
           ...
           // this.el.style.position = "absolute";
            this.el.style.position = "fixed"; // 固定在上方，隨 scrollbar 移動
...
// var PATH_STROKE_WIDTH = .4;
// var SELECTED_PATH_STROKE_WIDTH = 0.6;
var PATH_STROKE_WIDTH = .8;
var SELECTED_PATH_STROKE_WIDTH = 1.0;
...
var VALID_ATTRS = ['id', 'size', 'cluster', 'text','amount'];
...
// this.opts.color = d3.scale.category20();
  this.opts.color = options.colors;
...
// console.log('true : '+ cluster)
// }else{
// console.log('false : '+ cluster)
// }
...
// "4" : "limegreen",
      ...
      clustersObj["3"] = {};
      clustersObj["4"] = {};
      clustersObj["0"].color="cornflowerblue";
      clustersObj["1"].color="red";
      clustersObj["2"].color="yellow";
      clustersObj["3"].color="blueviolet";
      clustersObj["4"].color="limegreen";
    });
  ...
  /**
     * Given the current zoom and size of a node, tells if it's label should be displayed
     * 
     * @api private
     */
  ...
  pathStroke: function(d) {
    ...
    if(d.source.cluster==1 && d.target.cluster==1){
        // return "#cc0000";
        return "orangered";
    }else{
        return "#0000cc";
...
// }
  ...
  ffsRefreshPathUrlAndGNode:function(){
    // refresh url
//    this.parent.selectAll("path").attr("marker-end","url()");
//    this.parent.selectAll("path:not([stroke=transparent])").attr("marker-end",function(d){
//        if(!d.source){
//            return "url(#" + d + ")";
//        }
//        
//        if(d.source.cluster==1 && d.target.cluster==1){
//            return "url(#" + "historyArrowhead" + ")";
//        }else if(d.source.cluster==1){
//            return "url(#" + "arrowhead" + ")"; 
//        }
//        return "url(#" + "incomeArrowhead" + ")";
//    });
      this.parent.selectAll("path.marker").attr("marker-end","url()");
      this.parent.selectAll("path.marker:not([stroke=transparent])").attr("marker-end",function(d){
          if(!d.source){
              return "url(#" + d + ")";
          }
    ...
    // visibility hidden 沒用 QQ
    this.parent.selectAll("path").attr('display',null);
    this.parent.selectAll("path[stroke=transparent]").attr('display','none');
    
    // refresh node
    this.parent.selectAll(".node[display='none']").attr('display',null);
    var hideText = this.parent.selectAll("circle[style='fill: transparent; cursor: default; stroke: transparent;']");
    // 暫時先這樣寫
    hideText.each(function(d){
        d3.select(this.parentNode).attr('display','none');
    });	
  },
  ...
  render: function(scope) {
    this.history =scope.model.history;
    var self = this;

    this.init();
    // function circleRadius(d) {return self.radiusScale(self.getSize(d) || 1); }
    /* ffs modify */
    function circleRadius(d) {return (self.getSize(d) || 1)+5; }
    /* ffs modify end */
    ...
    function dragmove(d, i) {
       ...
       // self.handleCollisions();
        ...
        // 避免引發斥力 (拖拉到其他節點時，因斥力其他節點會移動)
        // self.refreshZoom();
    }

    function dragend(d, i) {
        d.fixed = true; // of course set the node to fixed so the force doesn't include the node in
    ...
    function releasenode(d) {
        nodeClickedOnce = false;
        d3.event.stopPropagation();
        d.fixed = false; // of course set the node to fixed so the force doesn't include the node
                            // in its auto positioning stuff
        // force.resume();
        clearTimeout(nodeClickEvent);
    }
    
    function findNodeTradeRelation(d){
        nodeClickedOnce = false;
        d3.event.stopPropagation();
        clearTimeout(nodeClickEvent);
        var node = d3.select(this).select('circle');
        scope.findNodeTradeRelation(d,node);
    }
      
    /* add for ffs */
    this.parent.append("defs").selectAll("marker")
            // .data(["suit", "licensing", "resolved"])
            .data(["arrowhead","incomeArrowhead","historyArrowhead"])
            .enter().append("marker")
    ...
    this.d3Path = this.parent.append("svg:g").selectAll("path")
      ...
      .attr("fill", "none")
//      .attr("testValue",function(d){return d.target.text;})
...
//      })
    ...
    this.d3MarkerPath = this.parent.append("svg:g").selectAll("path.marker")
...
//    .attr("class", function(d) { return "marker_only " + d.type; })
    ...
    var node = this.d3Nodes = this.parent.selectAll(".node")
      ...
      .on("click",bind(this,function(d) {
            ...
            nodeClickedOnce = true;
            var self = this, e = d3.event,forceId = this.focus(d.id);
            nodeClickEvent = setTimeout(function() {
                if(nodeClickedOnce){
                    ...
                    forceId.update();
                    self.emit("node:click", d);
                }
            }, 250);
          }))
      .on('dblclick', findNodeTradeRelation)
...
// console.log(force.alpha() +" : "+ defaults.forceAlphaLimit)
...
// self.d3Nodes.attr("transform", function(d) {
// console.log(d)
// });
  ...
  updateSpecialNodeColor:function(specialBans){
      this.d3Nodes.forEach(function(d,i) {
          for (var i = 0; i < d.length; i++) {
              d3.select(d[i]).each(function(d){
                  if(d.o_cluster!=undefined){
                      d.cluster = d.o_cluster;
                  }
              });
          }
      });
      if(specialBans.length>0){
          this.d3Nodes.forEach(function(d,i) {
              for (var i = 0; i < d.length; i++) {
                  d3.select(d[i]).each(function(d){
                      for(var i=0;i<specialBans.length;i++){
                          if(specialBans[i]==d.ban){
                              d.o_cluster = d.cluster;
                              d.cluster="3";
                              break;
                          }
                      }
      ...
      this.d3Circles.style("fill", bind(this, this.getClusterColor));
  },
  ...
  updateLinksPosition: function() {
    ...
    this.d3MarkerPath.attr("d", function(d) {
        var dx = d.target.x - d.source.x,
            dy = d.target.y - d.source.y,
            dr = Math.sqrt(dx * dx + dy * dy);
...
//        console.log(d);
        ...
        // var myText = pathInfo[key];
        
        // var obj = $.extend({}, d.target);
        
        obj = $.extend({}, pathInfo[d.source.ban+"-"+d.target.ban] || {});
        
        for(var key in obj){
            if(obj[key]==null || obj[key]==undefined || obj[key]===''){
                obj[key] = defaultEmptyStr;
            }
        }
        
        var fromNode = d.source.ban.indexOf("_")!=-1?d.source.ban:d.source.ban+"_"+d.source.name;
        var destNode = d.target.ban.indexOf("_")!=-1?d.target.ban:d.target.ban+"_"+d.target.name;
        var detailObj = detailInfo[d.source.ban+"-"+d.target.ban] || obj.details || [];
        var detailContent = "";
        
        if(d.source.ban.indexOf("_")==-1 && d.target.ban.indexOf("_")==-1){
            for(var i=0;i<detailObj.length;i++){
                for(var key in detailObj){
                    if(detailObj[key]==null || detailObj[key]==undefined || detailObj[key]===''){
                        detailObj[key] = defaultEmptyStr;
                    }
                }
                detailContent+="<tr>";
                detailContent+="<td style='padding-right:10px'>"+detailObj[i].itemDesc+"</td>";
//                detailContent+="<td>"+detailObj[i].subTotal+"</td>";
                detailContent+="<td>"+self.formatNumber(detailObj[i].subTotal)+"</td>";
                detailContent+="</tr>";
            }
...
//                "<div>下游: {{destNode}}</div>" +
              "<div>上游: "+fromNode+"</div>" +
              "<div>下游: "+destNode+"</div>" +
                "<div>營業稅申報金額: {{taxAmount}}</div>" +
                "<div>營業稅開立發票張數: {{invoiceCount}}</div>" +
                "<div>電子發票交易總金額: {{totalAmount}}</div>";
            detail = 
            "<div>" +
                "<table>" +
                    "<tr>" +
                        "<td style='padding-right:10px'>品項名稱</td>" +
                        "<td>金額小計</td>" +
                    "</tr>" +
                    detailContent +
                "</table>" +
            ...
            for(var i=0;i<detailObj.length;i++){
                ...
                detailContent+="<tr>";
                detailContent+="<td style='padding-right:10px'>"+detailObj[i].importedName+"</td>";
                detailContent+="<td style='padding-right:10px'>"+detailObj[i].importedDesc+"</td>";
                detailContent+="<td style='padding-right:10px'>"+detailObj[i].importedUnit+"</td>";
                detailContent+="<td style='padding-right:10px'>"+detailObj[i].quantity+"</td>";
//                detailContent+="<td style='padding-right:10px'>"+
//                (d.source.ban.indexOf("_")!=-1 ?detailObj[i].fromShorePrice:detailObj[i].leaveShorePrice)+
//                "</td>";
//                detailContent+=(d.source.ban.indexOf("_")!=-1 ?"<td>"+detailObj[i].priceTaxed+"</td>":"");
                detailContent+="<td style='padding-right:10px'>"+
                self.formatNumber((d.source.ban.indexOf("_")!=-1 ?detailObj[i].fromShorePrice:detailObj[i].leaveShorePrice))+
                "</td>";
                detailContent+=(d.source.ban.indexOf("_")!=-1 ?"<td>"+self.formatNumber(detailObj[i].priceTaxed)+"</td>":"");
                detailContent+="</tr>";
            }
            message = 
//                "<div>上游: {{fromNode}}</div>" +
//                "<div>下游: {{destNode}}</div>" +
                ...
                "<div>幣別: {{currency}}</div> " +
                "<div>"+
                (d.source.ban.indexOf("_")!=-1?"總項起岸價格: {{totalFromShorePrice}}"
                        :"總項離岸價格: {{totalLeaveShorePrice}}")+
                " </div> " +
                "<div>營業稅稅基合計: {{totalTaxBase}}</div> " +
                "<div>營業稅稅額合計: {{totalTaxAmount}}</div> ";
            detail = 
            "<div>" +
                "<table>" +
                    "<tr>" +
                        "<td style='padding-right:10px'>稅則代碼</td>" +
                        "<td style='padding-right:10px'>中文</td>" +
                        "<td style='padding-right:10px'>單位</td>" +
                        "<td style='padding-right:10px'>"+"" +
                        (d.source.ban.indexOf("_")!=-1?"核定數量":"出口數量")+
                        "</td>" +
                        "<td style='padding-right:10px'>"+
                        (d.source.ban.indexOf("_")!=-1?"起岸價格":"離岸價格")+
                        "</td>" +
                        (d.source.ban.indexOf("_")!=-1?"<td>完稅價格</td>":"") +
                    "</tr>" +
                    detailContent +
                "</table>" +
            "</div>";
        ...
        if(d.country==undefined || d.country==null || d.country===''){
            message = 
                "<div>公司名稱: {{text}}</div>" +
                "<div>營業狀態: {{status}}</div> " +
                "<div>稅籍異動原因: {{reasonDesc}}</div>"+
                "<div>統編: {{ban}}</div>" +
               //"<div>營業稅申報總金額(以月為單位): {{taxAmount}}</div>"+
                "<div>營業稅主行業別: {{mainTypeName}}</div>" +
                "<div>設立日期: {{establishedDate}}</div>"+
                "<div>營所稅行業別: {{taxTypesName}}</div> " +
                "<div>縣市: {{cityName}}</div>"+
                "<div>所屬分局稽徵所   : {{taxOffice}}</div> " +
                "<div>稅藉編號: {{taxCode}}</div>"+
                "<div>資本額: {{capital}}</div>" +
                "<div>質化/量化/食安風險: {{riskFlagsName}}</div>";    
        }else{
            message = 
                "<div>國別: {{countryName}}({{country}})</div>" +
                "<div>名稱: {{text}}</div>";
        }
     ...
     // 稅籍異常
        var exceptionArr = ['11','12','17','18','25','29','39','40','41','42','43','91'];
        if(exceptionArr.indexOf(obj.reason)!=-1){
            obj.status='稅籍異常';
        }else{
            obj.status='營業中';
        }
...
//            }
      ...
      // 千分位欄位
      var formatProperties = ["taxAmount","totalAmount","totalFromShorePrice"
                              ,"totalLeaveShorePrice","totalTaxBase","totalTaxAmount","capital"];
      
      for(var key in formatProperties){
          var property = formatProperties[key];
          if(obj.hasOwnProperty(property)){
              obj[property] = self.formatNumber(obj[property]);
          }
      }      
      
    // graph.tooltip(message+detail);
    self.tooltip(message+detail);
    ...
    this.showTooltip(offset, obj);
    this.emit("node:mouseover", obj, offset);
  ...
  /**
     * Tells if the graph has currently un-applied filters (filters that haven't been applied to the
     * view)
     * 
     * @api private
     */
  ...
  /**
     * Tells if the graph has currently applied filters (filters that have been applied to the view)
     * 
     * @api private
     */
  ...
  update: function() {
    ...
    this.ffsRefreshPathUrlAndGNode();
  },
  ...
  /**
     * Given the current state of the graph (focus and filters), tells if a node is visible.
     * 
     * @api private
     */
  ...
  /**
     * Will set the graph to focus on the node that matches the passed arg, the next time that
     * `update()` is called.
     * 
     * @api public
     * 
     * @param {Object|Function|Number|String}
     *            fn
     */
  ...
  filterByAmount: function(amount) {
    var self = this;
    var min = amount[0];
    var max = amount[1];
  ...
  /**
     * centers the graph on a point, if no point is passed, the mass center of the graph is used.
     * 
     * @api private
     * 
     */
  ...
  /**
     * centers the graph on a node by id, if no id is passed, the mass center of the graph is used.
     * 
     * @api public
     * 
     */
  ...
  getSize: function(node) {
   ...
   getAmount: function(node) {
    return this.nodeVal('amount', node) || 0;
  },
  ...
  getClusterColor: function(cluster) {
    if(cluster.ban == this.history.nodeBan){
        return "fuchsia";//fuchsia , violet
    }
    var c;
...
require.register("ffs/fisheye.js", function(exports, require, module){
...
require.alias("mbostock-d3/d3.js", "insights/deps/d3/d3.js");
require.alias("mbostock-d3/index-browserify.js", "insights/deps/d3/index-browserify.js");
require.alias("mbostock-d3/index-browserify.js", "insights/deps/d3/index.js");
...
require.alias("ffs/fisheye.js", "insights/fisheye.js");

if (typeof exports == "object") {
  module.exports = require("insights");
} else if (typeof define == "function" && define.amd) {
  define(function(){ return require("insights"); });
} else {
  this["Insights"] = require("insights");
}
